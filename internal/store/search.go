...
func (s *Store) Search(ctx context.Context, q string, scope *types.Scope, project string, limit int) ([]types.SearchHit, error) {
	return s.search(ctx, q, scope, project, false, limit)
}

// SearchScoped runs FTS with ListMemories project matching:
// coalesce(project_slug,”) = project. Empty project is "no slug", not every project.
func (s *Store) SearchScoped(ctx context.Context, q string, scope types.Scope, project string, limit int) ([]types.SearchHit, error) {
	return s.search(ctx, q, &scope, project, true, limit)
}

func (s *Store) search(ctx context.Context, q string, scope *types.Scope, project string, filterProject bool, limit int) ([]types.SearchHit, error) {
	...
	rows, err := s.Pool.Query(ctx, `
		...
		SELECT kind, id, title, summary, rank FROM (
			...
			WHERE m.status = 'active'
			  ...
			  AND ($3::text IS NULL OR coalesce(m.project_slug, '') = $3)
			UNION ALL
			...
			WHERE p.status = 'active'
			  ...
			  AND ($3::text IS NULL OR coalesce(p.project_slug, '') = $3)
		) hits
		ORDER BY rank DESC, title ASC, id ASC
		LIMIT $4
	`, q, scopeArg, projectArg, limit)
	if err != nil {
